...
class GrantDiscoveryEngine:
    ...
    async def find_matching_grants(self, 
        ...
        return {
            ...
            "average_success_probability": sum(g.success_probability for g in matching_grants) / len(matching_grants) if matching_grants else 0
        }
...
class GrantSuccessTracker:
    ...
    async def _update_success_metrics(self):
        ...
        submitted_applications = len([a for a in self.applications if a["status"] in [GrantStatus.SUBMITTED, GrantStatus.APPROVED, GrantStatus.REJECTED]])
