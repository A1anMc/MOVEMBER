...
class MovemberDataUploadSystem:
    ...
    def _extract_annual_reports_from_csv(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        funding_cols = [col for col in df.columns if any(
            word in col.lower() for word in ['fund', 'budget', 'amount', 'money', 'cost']
        )]
        ...
        reach_cols = [col for col in df.columns if any(
            word in col.lower() for word in ['reach', 'participant', 'user', 'member', 'beneficiary']
        )]
    ...
    def _save_uploaded_data(
        self, data_id: str, data_type: str, source_file: str, extracted_data: Dict[str, Any], validation_status: str):
    ...
    def get_uploaded_data(self, data_type: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        columns = ["id",
             "data_id", "data_type", "source_file", "upload_date", "data_format", "extracted_data", "validation_status", "currency", "spelling_standard", "created_at"]
